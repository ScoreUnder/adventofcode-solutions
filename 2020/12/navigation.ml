...
let navi_instructions =
  File.lines_of "input"
  |> map (fun line ->
         let instr = line.[0] and mag = int_of_string (String.lchop line) in
         (match instr with 'L' | 'R' -> assert (mag mod 90 = 0) | _ -> ());
         (instr, mag))
  |> List.of_enum
...
let manhattan_abs (x, y) = abs x + abs y
...
let part1 () = manhattan_abs (navigate 0 0 0 navi_instructions)

let part2 () = manhattan_abs (navigate_p2 0 0 (10, -1) navi_instructions)
